...
for ss in range(nsim):
    ...
    dat_sub = pd.read_csv(f"/Users/pixushi/Box/0-ftsvd/TEMPTED_paper/FARMM/simdata/realsim{ss+1}_pmiss20_depth500K.csv", header=0, index_col=0)
    dat_sub['SubjectID'] = 'id_' + dat_sub['SubjectID'].astype('str')
    dat_sub['study_day'] = dat_sub['study_day'].astype('int')
    meta_sub = dat_sub[['study_day', 'SubjectID']]

    count_sub = dat_sub.drop(dat_sub.columns[0:3], axis=1)
    ...
    if ss % 10 == 9:
        err.to_csv("/Users/pixushi/Box/0-ftsvd/TEMPTED_paper/FARMM/result/farmm_reconstruction_ctf_nomean_pmiss20.csv", index=True)
