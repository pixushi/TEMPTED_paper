---
title: "Simulation Based on FARMM Data - Identical Time Points"
author: "Pixu Shi"
date: "`r Sys.Date()`"
output:
  html_document:
...
# remove variables appearing in <5 samples
prop_tab <- prop_tab[,colSums(prop_tab!=0)>=5]
# remove samples having <5 taxa
prop_tab <- prop_tab[rowSums(prop_tab!=0)>=5,]
# remove time point 0 since it is not missing at random
meta_tab <- meta_tab[meta_tab$study_day>0,]
# intersect two data frames
sampleIDs <- intersect(rownames(prop_tab),rownames(meta_tab))
meta_tab <- meta_tab[sampleIDs,]
prop_tab <- prop_tab[sampleIDs,]
dim(prop_tab)
dim(meta_tab)
```
