...
```{r time_loading, fig.width = 3, fig.height=3}
...
p_sub <- plot_ly(A_hat, x=~`Component 1`, y=~`Component 2`, z=~`Component 3`, 
...
p_sub23 <- ggplot(data=A_hat, aes(x=`Component 2`, y=`Component 3`)) +
...
p_traj <- plot_feature_summary(ctf_traj[,c("Component 1","Component 2")], 
                     ctf_traj$week_int, 
                     ctf_traj$group,
                     bws=bws) + 
  xlab('Week') + theme_bw() + 
...
```{r ctf_time, fig.width = 3, fig.height=3}
...
```{r ctf_allplot, fig.width = 10, fig.height=3}
lay <- rbind(c(1,2,3,3), c(1,2,3,3), c(1,2,3,3), c(1,2,3,3), c(1,2,3,3), 
             c(4,5,5,5))
lgd1 <- get_legend(p_time)
lgd2 <- get_legend(p_sub23)
