...
ps_filtered <- filter_taxa(ps_filtered, function(x) sum(x!=0) > 0, TRUE)
...
![plot of chunk sub_loading](figure/sub_loading-1.png)
...
p_feat_ratio_summary <- plot_feature_summary(feature_mat_ratio1, 
  ...
  scale_x_continuous(breaks=seq(from=84,to=294,by=28)) + ylim(c(-8.1, 7.8))+
...
```r
ggsave('../figure_table/vaginal_Stanford_plot.pdf', width=10, height=2.5, plot=p_all)
```
