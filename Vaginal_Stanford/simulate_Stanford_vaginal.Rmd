...
rm(list=ls())
# for computing
#reticulate::use_condaenv(condaenv="/hpc/home/ps294/miniconda")
...
meta_tab2 <- dplyr::filter(meta_tab, day_of_life<390)
meta_tab2 <- meta_tab2[,c("day_of_life", "studyid", "delivery")]
for (ss in 1:nsim){
  print(ss)
  for (jj in 1:length(nkeep)){
    ...
    for (i in 1:nrow(metauni)){
      meta_sub <- dplyr::filter(meta_tab2, studyid==metauni$studyid[i])
      grd <- (0:ntime)*(390/ntime)
      sel <- NULL
      for (k in 1:ntime){
        sel <- rownames(meta_sub)[meta_sub$day_of_life<grd[k+1] & meta_sub$day_of_life>=grd[k]]
        if(length(sel)>0) sampleID_temp <- c(sampleID_temp, sample(sel, 1))
      }
    }
    meta_sub <- meta_tab2[sampleID_temp,]
    meta_sub$time_int <- floor(meta_sub$day_of_life/390*ntime)
...
for (jj in 1:length(nkeep)){
  ...
  for (ss in 1:nsim){
    ...
    c(roc_sub_glm_tempted[ss,jj], pr_sub_glm_tempted[ss,jj])
    c(roc_sub_rf_tempted[ss,jj], pr_sub_rf_tempted[ss,jj])
  }
}
...
## Run TCAM, save subject distance matrix and trajectory
...
for (ss in 1:nsim){
  ...
  for (jj in 1:length(nkeep)){
    ...
    metauni <- unique(meta_sub[,c("studyid", "delivery_ind")])
    # impute missing measurement by the nearest previous observation
    tab_miss <- table(meta_sub$studyid, meta_sub$time_int)
    for(nn in 1:nrow(tab_miss)){
      for (ii in which(tab_miss[nn,]==0)){
        fillin <- max(which(tab_miss[nn,1:(ii-1)]!=0))
        meta_fillin <- dplyr::filter(meta_sub, studyid==rownames(tab_miss)[nn], time_int==colnames(tab_miss)[fillin])
        meta_fillin$time_int <- colnames(tab_miss)[ii]
        meta_sub <- rbind(meta_sub, meta_fillin)
      }
    }
    count_sub <- count_tab[meta_sub$sampleid,]
    subdata <- format_tempted(count_sub, meta_sub$time_int, meta_sub$studyid, 
                            threshold=0.95, pseudo=0.5, transform='clr')
    metauni_sub <- metauni[names(subdata),]
...
for (jj in 1:length(nkeep)){
  ...
  for (ss in 1:nsim){
    ...
    #c(roc_sub_glm_tcam[ss,jj], pr_sub_glm_tcam[ss,jj])
    #c(roc_sub_rf_tcam[ss,jj], pr_sub_rf_tcam[ss,jj])
  }
}
...
```{r summary_sample_classification}
