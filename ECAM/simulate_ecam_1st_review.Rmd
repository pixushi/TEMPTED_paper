...
for (ss in 1:nsim){
  print(ss)
  # use the 9 time point simulated datasets
  jj <- length(nkeep)
  meta_sub <- read.csv(paste0("simdata/realsim", ss, "_ntime", nkeep[jj], ".csv"), header=T, row.names=1)
  count_sub <- count_tab[rownames(meta_sub),]
  meta_sub$studyid <- as.character(meta_sub$studyid)
  ...
  if (ss%%10==0){
    write.csv(R2_tempted_nomean, file="result/ecam_reconstruction_tempted_nomean.csv")
    write.csv(R2_tempted_mean, file="result/ecam_reconstruction_tempted_mean.csv")
  }
}
...
for (ss in 1:nsim){
  ...
  if (ss%%10==0){
    write.csv(R2_ftsvd_nomean, file="result/ecam_reconstruction_ftsvd_nomean.csv")
    write.csv(R2_ftsvd_mean, file="result/ecam_reconstruction_ftsvd_mean.csv")
  }
}
```
...
for (ii in 1:length(fname)){
  R2_csv <- read.csv(paste0("result/ecam_reconstruction_", fname[ii], ".csv"), header=T, row.names = 1)
